  ...
  return (
    <div className="flex items-center gap-2 text-[14px] whitespace-nowrap overflow-hidden overflow-ellipsis flex-shrink-1 min-w-[10rem]">
      {breadcrumbs?.map((breadcrumb, index) => {
        return (
          <div
            className="flex items-center gap-2 whitespace-nowrap overflow-hidden overflow-ellipsis"
            key={`breadcrumb-${index}-${breadcrumb.href}`}
          >
            {breadcrumb.href === undefined ? (
              <span className="text-[#00142B] font-semibold whitespace-nowrap overflow-hidden overflow-ellipsis">
                {breadcrumb.text}
              </span>
            ) : (
              <Link
            ...
            {index !== (breadcrumbs?.length || 0) - 1 && (
              <ChevronRightIcon className="w-3 h-3 text-[#263B62] stroke-[#263B62]" />
            )}
          </div>
        );
      })}
