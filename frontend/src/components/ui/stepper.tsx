...
type DefineStepperProps<Steps extends Stepperize.Step[]> = Omit<Stepperize.StepperReturn<Steps>, 'Scoped'> & {
  StepperProvider: (
  ...
  StepperDescription: (props: React.ComponentProps<'p'> & {asChild?: boolean}) => React.ReactElement;
...
const defineStepper = <const Steps extends Stepperize.Step[]>(...steps: Steps): DefineStepperProps<Steps> => {
  ...
  return {
    ...
    StepperNavigation: ({children, className, 'aria-label': ariaLabel = 'Stepper Navigation', ...props}) => {
      const {variant} = useStepperProvider();
      return (
        <nav aria-label={ariaLabel} role="tablist" className={cn('stepper-navigation', className)} {...props}>
          <ol className={listVariants({variant: variant})}>{children}</ol>
        </nav>
      );
    },
    StepperStep: ({children, className, icon, onlyIcon = false, isLoading = false, ...props}) => {
      ...
      const stepIndex: number = utils.getIndex(props.of);
      const step = steps[stepIndex];
      const currentIndex: number = utils.getIndex(current.id);
      ...
      if (variant === 'circle') {
        return (
          <li className={cn('stepper-step flex shrink-0 items-center gap-4 rounded-md transition-colors', className)}>
...
const StepperTitle = ({children, className, asChild, ...props}: React.ComponentProps<'h4'> & {asChild?: boolean}) => {
  const Comp = asChild ? Slot : 'h4';

  return (
    <Comp className={cn('stepper-step-title text-base font-medium text-[14px] font-semibold', className)} {...props}>
      {children}
    </Comp>
  );
};
...
const StepperDescription = ({children, className, asChild, ...props}: React.ComponentProps<'p'> & {asChild?: boolean}) => {
  const Comp = asChild ? Slot : 'p';

  return (
    <Comp className={cn('stepper-step-description text-sm text-muted-foreground', className)} {...props}>
      {children}
    </Comp>
  );
};
...
const listVariants = cva('stepper-navigation-list flex gap-1 w-[38px]', {
  variants: {
    variant: {
...
const onStepKeyDown = (e: React.KeyboardEvent<HTMLButtonElement>, nextStep: Stepperize.Step, prevStep: Stepperize.Step) => {
  const {key} = e;
  const directions = {
