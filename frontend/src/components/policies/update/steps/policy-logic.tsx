...
export const PolicyLogic = ({isLoading = false, policyLogicForm}: {policyLogicForm: UseFormReturn<PolicyLogicyFormValues>; isLoading?: boolean}) => {
  ...
  const [tempRule, setTempRule] = useState<undefined | {index: number; id: string}>();
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        description="Are you sure you want to remove this rule? This action cannot be undone."
        confirmButtonText="Remove Rule"
        onCancel={() => {
          setOpenConfirmModal(false);
          setTempRule(undefined);
        }}
        buttonConfirmProps={{color: 'negative'}}
