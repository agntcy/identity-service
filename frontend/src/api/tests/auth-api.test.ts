...
// Mock the AuthApi
vi.mock('../generated/identity/auth_service.swagger.api', () => {
...
import {AuthAPIClass} from '../services/auth-api';

describe('AuthAPIClass', () => {
  let api: AuthAPIClass;
  let mockAnalytics: AnalyticsBrowser;

  beforeEach(() => {
    vi.clearAllMocks();
    api = new AuthAPIClass({baseURL: config.API_HOST});
  ...
  describe('Initialization', () => {
    it('should create an instance with the correct base URL', () => {
      expect(api).toBeInstanceOf(AuthAPIClass);
    });

    it('should expose API methods from v1Alpha1', () => {
      expect(api.approveToken).toBeDefined();
    });
  });
