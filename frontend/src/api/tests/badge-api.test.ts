...
// Mock the BadgeApi
vi.mock('../generated/identity/badge_service.swagger.api', () => {
...
import {BadgeAPIClass} from '../services/badge-api';

describe('BadgeAPIClass', () => {
  let api: BadgeAPIClass;
  let mockAnalytics: AnalyticsBrowser;

  beforeEach(() => {
    vi.clearAllMocks();
    api = new BadgeAPIClass({baseURL: config.API_HOST});
  ...
  describe('Initialization', () => {
    it('should create an instance with the correct base URL', () => {
      expect(api).toBeInstanceOf(BadgeAPIClass);
    });

    it('should expose API methods from v1Alpha1', () => {
      expect(api.issueBadge).toBeDefined();
      expect(api.verifyBadge).toBeDefined();
    });
  });
