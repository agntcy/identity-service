...
// Mock the DevicesApi
vi.mock('../generated/identity/device_service.swagger.api', () => {
...
import {DevicesAPIClass} from '../services/devices-api';

describe('DevicesAPIClass', () => {
  let api: DevicesAPIClass;
  let mockAnalytics: AnalyticsBrowser;

  beforeEach(() => {
    vi.clearAllMocks();
    api = new DevicesAPIClass({baseURL: config.API_HOST});
  ...
  describe('Initialization', () => {
    it('should create an instance with the correct base URL', () => {
      expect(api).toBeInstanceOf(DevicesAPIClass);
    });

    it('should expose API methods from v1Alpha1', () => {
      expect(api.addDevice).toBeDefined();
      expect(api.registerDevice).toBeDefined();
      expect(api.listDevices).toBeDefined();
      expect(api.deleteDevice).toBeDefined();
      expect(api.testDevice).toBeDefined();
    });
  });
