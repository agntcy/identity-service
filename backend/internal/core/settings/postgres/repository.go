...
func (r *repository) UpdateIssuerSettings(
	...
	err = r.dbContext.Transaction(func(tx *gorm.DB) error {
		...
		case types.IDP_TYPE_DUO:
			...
			previousIdpSettingsID = ptrutil.Derefrence(existingSettings.DuoIdpSettingsID, uuid.UUID{})
			emptyIdpSettings = &DuoIdpSettings{}
		case types.IDP_TYPE_OKTA:
			existingSettings.OktaIdpSettings = model.OktaIdpSettings
			previousIdpSettingsID = ptrutil.Derefrence(existingSettings.OktaIdpSettingsID, uuid.UUID{})
			emptyIdpSettings = &OktaIdpSettings{}
		case types.IDP_TYPE_ORY:
			existingSettings.OryIdpSettings = model.OryIdpSettings
			previousIdpSettingsID = ptrutil.Derefrence(existingSettings.OryIdpSettingsID, uuid.UUID{})
			emptyIdpSettings = &OryIdpSettings{}
	...
	return existingSettings.ToCoreType(r.crypter), nil
}
