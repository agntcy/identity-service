...
test.describe("CNCF ID-JAG cross-app access + Gitea narrow-scoping demo", () => {
  ...
  test("renders the cross-app access UI with logical app names", async ({
    page,
  }) => {
    ...
    await expect(page.locator('input[name="scope"]:checked')).toHaveValue(
      "openid gitea:read",
    );
  });

  test("shows a sequence diagram with five message steps and Gitea", async ({
    page,
  }) => {
    ...
    await expect(page.locator("#tab-demo .diagram")).toContainText(
      "Mint ID-JAG",
    );
    await expect(page.locator("#tab-demo .diagram")).toContainText(
      "gitea:write",
  ...
  test("has a How it works tab explaining Keycloak config and VCs", async ({
    page,
  }) => {
  ...
  test("read-only run: lists repos but is denied the write (narrow scoping)", async ({
    page,
  }) => {
    ...
    await expect(page.locator(".badge.denied")).toHaveCount(1, {
      timeout: 45_000,
    });

    await expect(page.locator('[data-step="gitea-list"]')).toHaveClass(
      /\bok\b/,
    );
    await expect(page.locator('[data-step="gitea-create"]')).toHaveClass(
      /\bdenied\b/,
    );
