...
HYDRA_CONFIG_FILE="./contrib/quickstart/5-min/hydra.yaml"
...
do_setup_hydra() {
  ...
  # Modify the quickstart file with the current IP address
  # Change 127.0.0.1 to current_ip
  sed -i '' "s/127\.0\.0\.1/$current_ip/g" "$HYDRA_CONFIG_FILE"

  echo "Updated Hydra config file with current IP: $current_ip"
  echo "Hydra config file content:"
  cat "$HYDRA_CONFIG_FILE"
}
